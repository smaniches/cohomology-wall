...
| 7 | 20:1994, 21:2688 (acyclic-range formula) | 2688 | 14 | 15 | **21** | 20 |
...
the global-section complex exact. The `b=7` defects are the proven-formula
predictions; the dense `F_p` rank at `b=7` is a large run (about 24 GB), produced by
`scripts/run_b7_decisive.py` and checked by `scripts/verify_v04.py`. See `paper/`
...
└── results/
    ...
    └── b7_decisive_prediction.json ← b=7 (1994/2688) prediction + confirmed dims; dense rank pending
```
