# Independent recomputation of the intersection number, plus an edge-case sweep
# of the closed form over the full stated domain.
...
print("Intersection number recomputation (independent method):")
...
# 3. Edge case: does the rank formula stay non-negative and <= min(src,tgt)
...
print(f"\nEdge-case sweep over a_i in [4,15]^3 ({12**3} points):")
print(f"  formula violations (neg rank, neg defect, rank>min): {len(bad)}")
for b in bad[:10]: print("   ",b)

# 4. Boundary a_i=4 exactly (smallest allowed). Defect should be 3 at (4,4,4).
...
print(f"\n  Ceiling stress: max defect over [4,15]^3 = {maxd}  [ceiling claims 48]  {'OK' if maxd==48 else 'EXCEEDS — investigate'}")
